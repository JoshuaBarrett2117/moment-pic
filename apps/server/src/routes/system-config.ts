...
export const systemConfigRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch("/api/v1/system-config", async (request) => {
    const body = request.body as { enablePolling?: boolean; pollingInterval?: number; preloadBefore?: number; preloadAfter?: number; albumListItemMinWidthMobile?: number; albumListItemMinWidthDesktop?: number; albumDetailItemMinWidthMobile?: number; albumDetailItemMinWidthDesktop?: number };
